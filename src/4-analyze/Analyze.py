...
c_polder = conc_polder_2024.where(conc_polder_2024<2)
c_AM     = conc_AM_2024.where(conc_AM_2024<2)
c_Cl     = conc_Cl_2024.where(conc_Cl_2024<17)
...
# Concentration Cl at t = 20y
for i, (start, end) in enumerate(zip(starts, ends)):
    fig, ax = plt.subplots()
    CS = imod.select.cross_section_line(c_Cl, start=start, end=end)
    ax = CS.plot(levels=levels_conc, yincrease = False)
