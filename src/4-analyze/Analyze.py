...
conc_2024    = imod.idf.open(r"c:\projects\msc-thesis\data\4-output\conc\conc_202412312359_l*.idf")
...
# Cross section plot 2 - perpendicular to coastline
start_x2   = 77344.272
start_y2   = 462731.01 
...
# cross section 2 - heads

fig,ax = plt.subplots()
CS_25_1 = imod.select.cross_section_line(starting_head_regridded, start= start_2, end=end_2)
CS_25_1 = CS_25_1.plot(levels=levels_head, yincrease = False)
plt.title("Heads: CS perpendicular to coastline at t = 0 [d]")

fig, ax = plt.subplots()
CS_500_1 = imod.select.cross_section_line(heads_2024, start=start_2, end=end_2)
CS_500_1 = CS_500_1.plot(levels=levels_head, yincrease = False)
plt.title("Heads: CS perpendicular to coastline at t = 3650 [d]")

# cross section 2 - concentrations
...
# %% More cross sections!
# here we can create a series of starts and ends,

starts = []
ends = []

# This for loop creates a series of cross sections, given that the 
for i, (start, end) in enumerate(zip(starts, ends)):
    fig, ax = plt.subplots()
    CS_500_2 = imod.select.cross_section_line(conc_2024, start=start, end=end)
    CS_500_2 = CS_500_2.plot(levels=levels_conc, yincrease = False)
    plt.title("Conc: CS perpendicular to coastline at t = 3650 [d]")

    path = pathlib.Path(f"reports/images/cross_sections/head_cross_section_{i}.png")
