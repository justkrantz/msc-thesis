...
ghb_MM_SA = meta_ghb_2053.where(raster_MM==1).sum("layer") # Infiltration ponds, MM only
riv_OM_SA = OMriv_re.where(raster_MM==1).sum("layer")      # Infiltration ponds, OM only
#riv_OM_SA = OM_riv.where(raster_OM==1).sum("layer")       # Infiltration ponds, OM only

# OUT
drn_MM_SA = meta_drn_2053.where(raster_MM==1).sum("layer") # Surface runoff and phreatic 
drn_OM_SA = OMdrn_re_sum.where(raster_MM==1).sum("layer")  # Surface runoff and phreatic
#drn_OM_SA = OM_drn.where(raster_OM==1).sum("layer")       # Surface runoff and phreatic
...
WB_OM = ["riv", "drn", "well"]
axs = plt.bar(WB_OM, [riv_OM_SA.mean().compute(),   # Infiltration ponds
                      drn_OM_SA.mean().compute(),   # surface runoff (+pipe drainage?)
                      wel_OM_SA.mean().compute()])  # Wells
plt.title("SS Water Balance for OM")
plt.ylabel("$m^{3}$/d")
plt.grid()
path_3 = pathlib.Path(f"reports/images/Waterbalance_OM.png")
plt.savefig(path_3, dpi=300)
#%% Bar Charts MM
fig,axs = plt.subplots()
WB_OM = ["ghb", "rch", "drn", "well"]
axs = plt.bar(WB_OM, [ghb_MM_SA.mean().compute(),   # infiltration ponds
                      rch_MM_SA.mean().compute(),   # precipitation
                      drn_MM_SA.mean().compute(),   # surface runoff (+ pipe drainage?)
                      wel_MM_SA.mean().compute()])  # Wells
plt.title("SS Water Balance for MM")
plt.ylabel("$m^{3}$/d")
plt.grid()
path_3 = pathlib.Path(f"reports/images/Waterbalance_MM.png")
plt.savefig(path_3, dpi=300)
# %%
